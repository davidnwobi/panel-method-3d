...
TEST(APPLY_ADAJACENT_TEST_CIRCULAR, APPLY_ADJACENT_TEST_CIRCULAR) {
	...
	std::iota(numbers.begin(), numbers.end(), 1);
	std::vector<int> out(10);
	apply_adjacent_circular(numbers.begin(), numbers.end(), out.begin(), std::plus<int>());
...
TEST(RECTANGULAR_SOURCE_TEST_Compute_Task, RECTANGULAR_SOURCE_TEST) {
    ...
    Eigen::MatrixXd points = FileReaderFactory::make_file_reader("dat", " ", true)->read_data(testDataLoc.string()  + "/rectangular_source_test.txt");
	...
	ComputeTask computeTask = createInfluenceComputeTask(PanelGeometry(surfaces[0]), evalPoints, 0, idxs);	
	
	computeTask.face.points = surfaces[0].mPoints;
	computeTask.indices = std::move(idxs);
	computeTask.points = evalPoints.mEvalPoints;
	EXPECT_TRUE(SourceP<false>::calcInfluence(computeTask).isApprox(points.col(3), 1e-6));
}
TEST(RECTANGULAR_DOUBLET_TEST_Compute_Task, RECTANGULAR_DOUBLET_TEST) {

	std::filesystem::path filePath(testDataLoc.string() + "/singlepanel00.txt");
	auto [surfaces, wakes] = readConvertedComponentsFromFile(filePath);
    Eigen::MatrixXd points = FileReaderFactory::make_file_reader("dat", " ", true)->read_data(testDataLoc.string()  + "/rectangular_doublet_test.txt");
	...
	// computeTask.indices = std::move(idxs);
	// computeTask.points = evalPoints.mEvalPoints;
	EXPECT_TRUE(DoubletP<false>::calcInfluence(computeTask).isApprox(points.col(3), 1e-6));
}
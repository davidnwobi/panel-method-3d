...
Eigen::Array3Xf colwiseCross(const Eigen::Ref<const Eigen::Array3Xf> &A,
                             const Eigen::Ref<const Eigen::Array3Xf> &B) {
  Eigen::Array3Xf out(3, A.cols());
  auto colwiseCrossImpl =
      [](const Eigen::Ref<const Eigen::Vector3f> &a,
         const Eigen::Ref<const Eigen::Vector3f> &b) -> Eigen::Vector3f {
    return a.cross(b);
  };

...
Derived rotate_2f_about_origin(const Eigen::MatrixBase<Derived> &points2f,
                               float angle_d) {
...
void rotate_3f_about_origin(Eigen::Ref<Eigen::ArrayX3f> points3f,
                            float angle_d) {
  Eigen::MatrixXf points = rotate_2f_about_origin(
      (Eigen::MatrixXf(2, points3f.rows()) << points3f.col(0).transpose(),
       points3f.col(2).transpose())
          .finished(),
      angle_d);
  points3f.col(0) = points.row(0).transpose();
  points3f.col(2) = points.row(1).transpose();
}
...
void rotate_points_about_start(Eigen::Ref<Eigen::ArrayX3f> points3f,
  ...
  Eigen::RowVector3f original_loc(points3f(0, 0), 0, points3f(0, 2));
  points3f = points3f.rowwise() - original_loc.array(); // translate to origin
  rotate_3f_about_origin(points3f, angle_d);
  points3f = points3f.rowwise() + original_loc.array(); // translate from origin
}
